// Command selector exposes the Selector agent as a standalone A2A server.
//
...
func main() {
	...
	sel := selector.New(vc, agentCfg.SelectorModel)

	handler := a2asrv.NewHandler(sel)
	...
	slog.Info("A2A selector server listening", "addr", *addr)
