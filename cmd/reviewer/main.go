// Command reviewer exposes the Reviewer agent as a standalone A2A server.
//
...
func main() {
	...
	rev := reviewer.New(vc, agentCfg.ReviewerModel)

	handler := a2asrv.NewHandler(rev)
	...
	slog.Info("A2A reviewer server listening", "addr", *addr)
